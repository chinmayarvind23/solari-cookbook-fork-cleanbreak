  ...
  return page.evaluate((expectedUrl) => {
    ...
    const renewalStatus =
      unique<Billing["renewalStatus"]>(
        values("auto-renewal", "auto renewal", "automatic renewal").map((s) =>
    ...
    const absent = next.some((s) =>
      /^(none|no upcoming charges|no future charges|no next payment)$/i.test(s),
    )
    const amounts = next
    ...
    const dates = next.map(
      (s) => isoDate(s) ?? isoDate(s.match(/\b\d{4}-\d{2}-\d{2}\b/)?.[0] ?? ""),
    )
    ...
    return {
      ...
      billing: {
        ...
        accessUntil: unique(values("access until", "trial ends").map(isoDate)),
      },
    }
  }, expectedUrl)
}
...
export async function verifyMiroDOM(
  ...
  try {
    ...
    return {
      ...
      identityChecks,
      authenticated: second.pageMatched && second.heading && !second.dialog,
      confidence: 1,
      surface: second.heading && !second.dialog ? "BILLING_PAGE" : "UNKNOWN",
      target: "UNKNOWN",
      targetCount: 0,
      x: 0,
      y: 0,
