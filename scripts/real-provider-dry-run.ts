...
import { realProviderExecutor } from "@/lib/desktop/config"

async function browserDryRun() {
...
if (realProviderExecutor() === "desktop") {
  const { desktopDryRunCommand } =
    await import("./real-provider-desktop-dry-run")
  await desktopDryRunCommand()
} else await browserDryRun()
